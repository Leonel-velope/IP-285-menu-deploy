...
var APP_com_domain_app_MyAwesomeApp = (function () {
  ...
  const normalizeColorToARGB = color => {
    ...
    if (isString$1(targetColor) && check.test(targetColor)) {
      ...
      targetColor = (hex.length === 8 ? "0x".concat(hex) : "0xff".concat(hex)) * 1;
    }
              ...
              setTimeout((() => {
                  ...
                  if (this.flex && this.flex._layout && this.flex._layout._lineLayouter && this.flex._layout._lineLayouter._lines) {
                      ...
                      this.flex._layout._lineLayouter._lines.forEach((line => {
                          totalHeight += Object.entries(line.items).sort(((a, b) => b[1].h - a[1].h))[0][1].h;
                      }));
  ...
  class CardComponent extends lng.Component {
    constructor() {
      super(...arguments);
      ...
      return {
        Card: {
          ...
          Image: {
            ...
            h: 200,
            x: 0.7,
            src: Utils.asset('images/noPoster.png')
          },
          Details: {
            rect: true,
            w: 350,
    ...
    _handleEnter() {
      Router.navigate("details/".concat(this._id));
    }
    _focus() {
      this.focused = true;
    }
    ...
    set focused(isFocused) {
      const cardColorLabel = isFocused ? 'focus' : 'unfocused';
      const scale = isFocused ? 1.2 : 1;
      this._card.patch({
        color: Colors(cardColorLabel).get()
      });
      this._details.patch({
        color: Colors(cardColorLabel).get()
      });
      this._card.setSmooth('scale', scale);
    }
    set movieID(value) {
      this._id = value;
    }
    ...
    set movieImage(value) {
      this._card.patch({
        Image: {
          src: value ? value : Utils.asset('images/logo.png')
        }
      });
    }
  }
  ...
  const options = {
    ...
    headers: {
      ...
      Authorization: "Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI4MmRlZWY0M2NjZGQzZjhiZDZmYjBhZTViMzQ4OTUxZSIsIm5iZiI6MTcyNzI2OTczMi42OTU0ODcsInN1YiI6IjY2YWRiM2NmOGM0YmJkYzYxMmIwYTRkMyIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.moBZkwgXpjK-n8bqGKavxt4XpF30u7-dzMKCdP7JoO8"
    }
  };
  const imageUrlS = {
    backdropURL: 'https://image.tmdb.org/t/p/w500',
    backgroundURL: "https://image.tmdb.org/t/p/w1920_and_h800_multi_faces"
  };
  async function fetchMovies(id, page) {
    try {
      let response;
  ...
  async function fetchMoviesByGenre(id, page) {
    try {
      let response;
      response = await fetch("https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=en-US&page=".concat(page, "&sort_by=popularity.desc&with_genres=").concat(id), options);
      const data = await response.json();
      return data;
    } catch (error) {
      console.error('There has been a problem with your fetch operation:', error);
  ...
  const movieCollections = [{
    genreTitle: "Trending Movies"
  }, {
    genreTitle: "Romance Movies",
    genreId: 10749
  }, {
    genreTitle: "Animation Movies",
    genreId: 16
  }, {
  ...
  */
    ...
    return new Promise((resolve, reject) => {
      Transport.send('device', 'version')
        .then((v) => {
          ...
          v.sdk.readable = 'Firebolt Core SDK 1.4.0';
          resolve(v);
        })
        .catch((error) => {
          reject(error);
  ...
  class Platform {
    ...
    async init() {
      const [locale, locality, screenResolution, distributor, accountId] = await Promise.all([Localization.locale(), Localization.locality(), Device.videoResolution(), Device.distributor(), Account.id()]);
      this._locale = locale;
      this._locality = locality;
      this._width = screenResolution[0];
      this._height = screenResolution[1];
      this._distributorId = distributor;
  ...
  class HomePage extends lng.Component {
    constructor() {
      ...
      this._parentColumn = this.getByRef('ParentColumn');
      this._rectangleResolution = this.getByRef('RectangleResolution');
      this.filteredMovies = false;
      this.page = 1;
    }
    static _template() {
      return {
        ...
        RectangleResolution: {
          ...
          y: 40,
          rect: true,
          color: Colors('deviceInfBackground').get(),
          InfoText: {
            y: 50,
            x: 450,
            mount: 0.5,
    ...
    async _init() {
      this.createMovieList(movieCollections);
      Lifecycle.ready();
    }
    set params(args) {
      if (!args.genreId || args.genreId === 'all') {
        this.filteredMovies = false;
        this.createMovieList(movieCollections);
      } else {
        this.page = 1;
        let genreMovieCollection = [{
          genreTitle: 'Search Results',
          genreId: Number(args.genreId)
        }];
        this.filteredMovies = true;
        this.createMovieList(genreMovieCollection);
      }
    }
    _firstActive() {
      const locale = Platform$1.locale;
      const locality = Platform$1.locality;
      const width = Platform$1.width;
      const height = Platform$1.height;
      const distributorId = Platform$1.distributorId;
      const accountId = Platform$1.accountId;
      this._rectangleResolution.patch({
        InfoText: {
          text: {
            text: "Session information: userId:".concat(accountId, " | ").concat(locale, " ").concat(locality, " | ").concat(width, " X ").concat(height, " | ").concat(distributorId)
          }
        }
      });
    }
    async createMovieList(movieCollections) {
      try {
        let movieImage;
        const movieCollection = await Promise.all(movieCollections.map(async movieInfo => {
          let movieRes;
          if (this.filteredMovies) {
            ...
            movieRes = await fetchMovies(movieInfo.genreId);
          }
          const movieList = movieRes.results.map((movie, index) => {
            const {
              id,
              title,
              overview,
            ...
            if (backdrop_path) {
              movieImage = "".concat(imageUrlS.backdropURL).concat(backdrop_path);
            } else {
              movieImage = Utils.asset('images/noPoster.png');
            }
            ;
            return {
              type: CardComponent,
  ...
  class DetailsPage extends lng.Component {
    constructor() {
      super(...arguments);
      this._backGround = this.getByRef('Background');
      this._movieDetails = this.getByRef('MovieDetails');
      this._movieOptions = this._movieDetails.getByRef('MovieOptions');
    }
    static _template() {
      return {
        Background: {
    ...
    set params(args) {
      this.findMovieById(args.id);
    }
    async findMovieById(movieID) {
      const movieRes = await fetchMovieById(movieID);
      this.movieData = movieRes;
    }
    set movieData(data) {
      let {
        backdrop_path,
      ...
      this._movieDetails.patch({
        MovieInformation: {
          ...
          Status: {
            text: {
              text: "Status: ".concat(status)
            }
          },
          ReleaseDate: {
            text: {
              text: "Release Date: ".concat(formattedReleaseDate)
            }
          }
        }
      });
    }
    parseMovieReleaseDate(date) {
      const [year, month, day] = date.split('-');
      return "".concat(month, "/").concat(day, "/").concat(year);
    }
    _handleEnter() {
      var _a;
      if (((_a = this._movieOptions) === null || _a === void 0 ? void 0 : _a.selectedIndex) > 0) {
        Router.back();
      } else {
        return false;
      }
  ...
  class MenuComponent extends lng.Component {
    constructor() {
      super(...arguments);
      this._menu = this.getByRef('Menu');
      this._menuOptions = this._menu.getByRef('MenuOptions');
    }
    static _template() {
      return {
        Menu: {
    ...
    _init() {
      this.optionsCollection(movieGenres);
    }
    async optionsCollection(movieOptions) {
      const options = movieOptions.map(option => {
        return {
          type: Button,
          title: option.genreTitle,
          fixed: true,
          w: 370,
          selected: true,
      ...
      this._menuOptions.items = options;
    }
    _getFocused() {
